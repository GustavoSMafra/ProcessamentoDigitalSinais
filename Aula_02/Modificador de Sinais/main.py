...
def TelaIncial():
    layout = [
        [sg.Button('Média Móvel', button_color=('white', 'darkgreen'), size=(30, 1))],
        [sg.Button('Delay', button_color=('white', 'darkgreen'), size=(30, 1))],
        [sg.Button('Eco', button_color=('white', 'darkgreen'), size=(30, 1))],
        [sg.Button('Sair', button_color=('white', 'darkgreen'), size=(30, 1))],
        [sg.Text('\n\n\n\nAutor: Gustavo da Silva Mafra')]
    ]

    janela = sg.Window('Modificação de sinal', size=(300, 300),element_justification='c').layout(layout)
    event, value = janela.read()

    while True:
        if event == sg.WINDOW_CLOSED:
            break
        elif event == 'Média Móvel':
            janela.close()
            MediaMovel()
            break
        elif event == 'Delay':
